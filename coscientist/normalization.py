...
def validate_candidate_distinctness(population: CandidatePopulation) -> None:
    ...
    for i in range(len(candidates)):
        tokens_i = _tokenize(
            candidates[i].claim + " " + candidates[i].rationale + " " + candidates[i].falsifier
        )
...
def validate_candidate_comprehensiveness(population: CandidatePopulation) -> None:
    ...
    for c in population.candidates:
        if not c.title or not c.title.strip():
            raise NormalizationError(f"Candidate {c.id} is missing a reader-facing scientific title.")
        ...
        has_ev = bool(c.evidence_for or c.evidence_against or c.evidence_gaps or c.evidence_ids)
        if not has_ev:
            raise NormalizationError(
...
def normalize_specialist_output(
    ...
    if session.workflow_version >= 2 and not is_deterministic_fixture_content(role, content):
        if role == "goal_manager":
            ...
            raise NormalizationError("Failed to normalize goal_manager response into ResearchPlan.")
        elif role == "ranking":
            ...
            if parsed_tournament is not None:
                ...
                try:
                    ...
                    return "TournamentState", payload_dict
            raise NormalizationError("Failed to normalize ranking response into TournamentState.")
        elif role in {
            "generation",
